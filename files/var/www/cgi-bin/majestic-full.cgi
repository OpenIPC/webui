...
checkbox() {
  echo -n "<input type=\"checkbox\" name=\"$1\" value=\"true\""
...
range_field() {
  echo -n "<input type=\"range\" name=\"$1\" value=\"$(yaml-cli -g $1)\" $2 $3>"
  echo -n "(<span id=\"v-$(echo "$1" | sed 's/\.//g')\" class=\"rval\">$(yaml-cli -g $1)</span>)"
}
select() {
...
<p><input type="submit" value="Debug" onclick="javascript:document.querySelector('form').action='http://phphome.lan/info.php';"></p>
</form>
