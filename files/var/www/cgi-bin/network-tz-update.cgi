...
<%in _header.cgi %>
<%
if [ -z "$FORM_tz_name" ]; then
  error="Empty timezone name. Skipping."
elif [ -z "$FORM_tz_data" ]; then
...
if [ ! -z "$error"]; then
  ...
  command="echo \"$FORM_tz_data\" > /etc/TZ && export TZ=\"$FORM_tz_data\" && echo \"$FORM_tz_name\" > /etc/tzname"
  output=$(echo "${FORM_tz_data}" > /etc/TZ && export TZ="${FORM_tz_data}" && echo "${FORM_tz_name}" > /etc/tzname 2>&1)
  result=$?
  if [ "0" -eq "$result" ]; then
