...
for line in $mj; do
  ...
  case "$type" in
    boolean)
      ...
      echo -n "<div class=\"form-check form-switch\">" \
        ...
        "<label class=\"form-check-label\" for=\"${name}\">${label//_/ }</label>" \
        "</div>"
    ...
    hidden)
      ...
      ;;
    number)
      echo -n "<div class=\"col-md-7\">" \
        "<label class=\"form-label\" for=\"${name}\">${label//_/ }</label>" \
        "</div>" \
        "<div class=\"col-md-5\">" \
    ...
    range)
      ...
      if [ ! -z $(echo "${options}" | grep -E auto) ]; then
        ...
        echo -n "<span class=\"input-group-text\">" \
          "<label>" \
          "<input class=\"form-check-input auto-value\" type=\"checkbox\" data-for=\"${name}\" data-value=\"${default}\"${checked}>" \
          " auto" \
          "</label>" \
          "</span>"
      fi
      [ "auto" = "$value" ] && readonly=" readonly" || readonly=""
      echo -n "<input class=\"form-control text-end range\" type=\"text\" name=\"${name}\" id=\"${name}\" value=\"${value}\" placeholder=\"${placeholder}\" data-units=\"${units}\"${readonly}>"
      [ -n "$units" ] && echo -n "<span class=\"input-group-text\">${units}</span>"
      echo -n "</div></div>"
      [ -n "$hint" ] && echo -n "<p class=\"hint text-secondary\">${hint//_/ }</p>"
      ;;
