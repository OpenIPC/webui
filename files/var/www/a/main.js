...
(function () {
    function initAll() {
        ...
        if ($('pre#output[data-cmd]')) {
            const el = document.querySelector('pre#output[data-cmd]');
            ...
            async function run() {
                for await (let line of makeTextFileLineIterator('/cgi-bin/jrun.sh?cmd=' + btoa(el.dataset["cmd"]))) el.textContent += line + '\n';
            }
            run()
        }
    }
